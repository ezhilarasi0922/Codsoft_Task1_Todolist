...
class ToDoApp:
    ...
    def create_widgets(self):
        # Frame
        self.frame = Frame(self.master,bg="#DCBFFF")
        ...
        # Task Entry
        self.task_entry = Entry(self.frame, width=25,font=("Helvetica",11), bg="#FACBEA",fg="black",selectbackground="#B99470")
        self.task_entry.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)

        # Add Button
        self.add_button = Button(self.frame, text="Add Task", command=self.add_task,bg="#B0578D",fg="#FAEED1", font=("Helvetica",11,"bold"), cursor='hand2',  takefocus=False)
        self.add_button.grid(row=0, column=2, padx=5, pady=5, sticky=tk.W)

        # Task Listbox
        self.task_listbox = tk.Listbox(self.frame, width=50, height=10,font=("Helvetica",11,"bold"), bg="#FACBEA", fg="black",activestyle="none", selectbackground="#B99470")
        self.task_listbox.grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky=tk.W+tk.E+tk.N+tk.S)

        # Delete Button
        self.delete_button = Button(self.frame, text="Delete Task", command=self.delete_task,bg="#B0578D",fg="#FAEED1",font=("Helvetica",11,"bold"), cursor='hand2',  takefocus=False)
        self.delete_button.grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)

        # Mark as Complete Checkbox
        self.complete_button = Button(self.frame, text="Mark as completed", command=self.mark_complete,bg="#B0578D",fg="#FAEED1",font=("Helvetica",11,"bold"),  cursor='hand2',  takefocus=False)
        self.complete_button.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)

        # Completed Tasks
        self.completed_tasks = Button(self.frame, text="Refresh completed Tasks", command=self.tasks_completed,bg="#B0578D",fg="#FAEED1",font=("Helvetica",11,"bold"),  cursor='hand2',  takefocus=False)
        self.completed_tasks.grid(row=2, column=2, padx=5, pady=5,sticky=tk.W)

        # New Frame for Completed Tasks
        self.completed_frame = Frame(self.master,bg="#DCBFFF")
        self.completed_frame.grid(row=1, column=0, sticky=tk.W + tk.E + tk.N + tk.S)

        # Completed Tasks Listbox
        self.completed_listbox = tk.Listbox(self.completed_frame, width=50, height=5,font=("Helvetica",11,"bold"), bg="#FACBEA",fg="black",activestyle="none", selectbackground="#B99470")
